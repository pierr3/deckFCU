...
async function updateData(context: WillAppearEvent<NullSettings>) {
  ...
  const freq = Math.round(
    simDataProvider.getDatarefValue(DatarefsType.READ_COM2_ACTIVE)
  );

  const vol = Math.round(
    simDataProvider.getDatarefValue(DatarefsType.READ_WRITE_COM2_VOLUME) * 100
  );
...
export class Com2ActiveDial extends SingletonAction<NullSettings> {
